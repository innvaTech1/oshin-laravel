...
<form id="productModalFormId-{{ $product->id }}">
    <div class="wsus__quentity">
        ...
        <h3 class="d-none">{{ $currencySetting->currency_icon }}<span
                id="productModalPrice-{{ $product->id }}">{{ sprintf('%.2f', $productPrice) }}</span></h3>
                ...
                @foreach ($productVariants as $productVariant)
                    @php
                        $items = App\Models\ProductVariantItem::orderBy('is_default', 'desc')
                            ->where(['product_variant_id' => $productVariant->id, 'product_id' => $product->id])
                            ->get();
                    @endphp
                    @if ($items->count() != 0)
    ...
    <ul class="wsus__button_area">
        ...
        <li><a class="buy_now" href="javascript:;"
                onclick="addToBuyNow('{{ $product->id }}')">{{ __('user.buy now') }}</a></li>
        <li><a href="javascript:;" onclick="addToWishlist('{{ $product->id }}')"><i class="fal fa-heart"></i></a>
        </li>
...
<p class="brand_model"><span>{{ __('user.Brand') }} :</span> <a
        href="{{ route('product', ['brand' => $product->brand->slug]) }}">{{ $product->brand->name }}</a></p>
