        ...
        <section class="section">
          <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
              ...
              <div class="breadcrumb-item">{{__('State')}}</div>
            </div>
          </div>

          <div class="section-body">
            <a href="{{ route('admin.state.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> {{__('Add New')}}</a>
            <div class="row mt-4">
                <div class="col-12">
                  <div class="card">
                    <div class="card-body">
                                ...
                                @foreach ($states as $index => $state)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @php
                                                $isUser = $users->where('state_id', $state->id)->count();
                                                $isBilling = $billingAddress->where('state_id', $state->id)->count();
                                                $isShipping = $shippingAddress->where('state_id', $state->id)->count();
                                            @endphp
                                            @if ($state->cities->count() == 0 && $isUser == 0 && $isBilling == 0 && $isShipping == 0)
