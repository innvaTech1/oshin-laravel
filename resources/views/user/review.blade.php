            ...
            <div class="dashboard_content mt-2 mt-md-0">
                ...
                <div class="wsus__dashboard_review">
                    <div class="row">
                            ...
                            <div class="col-xl-6">
                                <div class="wsus__dashboard_review_item">
                                    <div class="wsus__dash_rev_img">
                                        <img src="{{ asset($review->product->thumb_image) }}" alt="product"
                                            class="img-fluid w-100">
                                    </div>
                                    <div class="wsus__dash_rev_text">
                                        <h5><a
                                                href="{{ route('product-detail', $review->product->slug) }}">{{ $review->product->short_name }}</a>
                                        ...
                                        <ul>
                                            <li><a href="#" data-bs-toggle="collapse"
                                                    data-bs-target="#flush-collapseOne-{{ $review->id }}"><i
                                                        class="fal fa-edit"></i> {{ __('user.edit') }}</a></li>
                                        </ul>
                                        <div class="accordion accordion-flush" id="accordionFlushExample">
                                            <div class="accordion-item">
                                                <div id="flush-collapseOne-{{ $review->id }}"
                                                    class="accordion-collapse collapse"
                                                    aria-labelledby="flush-collapseOne-{{ $review->id }}"
                                                    data-bs-parent="#accordionFlushExample">
                                                    <div class="accordion-body">
                                                        <form action="{{ route('user.update-review', $review->id) }}"
                                                            method="POST">
                                                            @csrf
                                                            <div class="wsus__riv_edit_single">
                                                                <i class="fas fa-star"></i>
                                                                <select class="select_2" name="rating">
                                                                    <option {{ $review->rating == 1 ? 'selected' : '' }}
                                                                        value="1">1</option>
                                                                    <option {{ $review->rating == 2 ? 'selected' : '' }}
                                                                        value="2">2</option>
                                                                    <option {{ $review->rating == 3 ? 'selected' : '' }}
                                                                        value="3">3</option>
                                                                    <option {{ $review->rating == 4 ? 'selected' : '' }}
                                                                        value="4">4</option>
                                                                    <option {{ $review->rating == 5 ? 'selected' : '' }}
                                                                        value="5">5</option>
                                                                </select>
                                                            </div>
                        ...
                        @else
                            <div class="col-12">
                                <p class="text-center text-danger display-5">{{ __('No Review Found') }}</p>
                            </div>
                        @endif
