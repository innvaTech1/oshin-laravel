                                ...
                                <div class="accordion-item">
                                    ...
                                    <div id="collapseOne" class="accordion-collapse collapse show"
                                        ...
                                        <div class="accordion-body">
                                            <ul>
                                                <li><a class="categoryForSearch" href="javascript:;"
                                                        data-category="0">{{ __('user.All Categories') }}</a></li>
                                                @foreach ($productCategories as $productCategory)
                                                    <li><a class="categoryForSearch" href="javascript:;"
                                                            data-category="{{ $productCategory->slug }}">{{ $productCategory->name }}</a>
                                                    </li>
                                                @endforeach
                                                <input type="hidden" name="category" value=""
                                                    id="category_id_for_search">
                                        ...
                                        <div class="accordion-body">
                                            @foreach ($brands as $brand)
                                                <div class="form-check">
                                                    <input name="brands[]" class="form-check-input brand_item"
                                                        type="checkbox" value="{{ $brand->id }}"
                                                        id="flexCheckDefault11-{{ $brand->id }}">
                                                    <label class="form-check-label"
                                                        for="flexCheckDefault11-{{ $brand->id }}">
                                                        {{ $brand->name }}
                                                    </label>
                                                </div>
                                            @endforeach
                                ...
                                @foreach ($variantsForSearch as $variantForSearch)
                                    <div class="accordion-item">
                                        <h2 class="accordion-header" id="headingThree2-{{ $variantForSearch->id }}">
                                            <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                                data-bs-target="#collapseThree2-{{ $variantForSearch->id }}"
                                                aria-expanded="false" aria-controls="collapseThree">
                                                {{ $variantForSearch->name }}
                                            </button>
                                        </h2>

                                        <div id="collapseThree2-{{ $variantForSearch->id }}"
                                            class="accordion-collapse collapse show"
                                            aria-labelledby="headingThree2-{{ $variantForSearch->id }}"
                                            data-bs-parent="#accordionExample">
                                            <div class="accordion-body">
                                                @php
                                                    $variantItemsForSearch = App\Models\ProductVariantItem::groupBy(
                                                        'name',
                                                    )
                                                        ->select('name', 'id')
                                                        ->where('product_variant_name', $variantForSearch->name)
                                                        ->get();
                                                @endphp

                                                @foreach ($variantItemsForSearch as $index => $variantItemForSearch)
                                                    <div class="form-check">
                                                        <input class="form-check-input variant_item_search"
                                                            type="checkbox" name="variantItems[]"
                                                            value="{{ $variantItemForSearch->name }}"
                                                            id="{{ $variantForSearch->id }}-{{ $variantItemForSearch->id }}-{{ $variantItemForSearch->name }}">
                                                        <label class="form-check-label"
                                                            for="{{ $variantForSearch->id }}-{{ $variantItemForSearch->id }}-{{ $variantItemForSearch->name }}">
                        ...
                        <div id="loadeer-hidden-content" class="d-none">
                            <div class="row">
                                <div class="col-12 mt-3">
                                    <div class="preloader">
                                        <img src="{{ asset('user/images/gif.gif') }}" alt="loader"
